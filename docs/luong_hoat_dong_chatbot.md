...
| `wp` | `POST /chatbot/upload/wp` | Hostname URL, bỏ `www.`, thay `.` bằng `_` | `https://e.vnexpress.net/...` → `e_vnexpress_net` |

Ví dụ request dùng website đã nạp:

```json
{
  "user_id": "user-123",
  "chat_input": "Trang này nói gì về công nghệ?",
  "selector_choices": "e_vnexpress_net",
  "reasoning_type": "wp"
}
```
...
Ví dụ khớp nguồn: PDF có đường dẫn `C:\data\bao-cao.pdf` được lưu vào `db/chroma_db_pdf_ollama/bao-cao`; khi chat phải dùng `reasoning_type: "pdf"` và `selector_choices: "bao-cao"`. Với URL `https://www.example.com/docs`, tên được chuẩn hóa thành `example_com`, nên index là `db/chroma_db_wp_ollama/example_com`.
...
1. Endpoint tạo `file_id` từ tên file không có phần mở rộng (PDF/Markdown), hoặc `website_name` từ hostname URL (web).
...
7. Endpoint trả `{ "message": "Document created successfully." }` hoặc thông báo nguồn đã tồn tại/lỗi.
